...
def display_grid(grid, size):  # (0, 0) is top right
    for y in range(size):
        row = []
        for x in range(size):
            row.append(grid[y][x])
...
def add_red_tiles(red_coords, size):
    grid = []
    for y in range(size):
        row = []
        for x in range(size):
...
def add_green_tiles(green_coordinates, size):
    grid = []
    for y in range(size):
        row = []
        for x in range(size):
...
def fill_green_coords(grid, green_coordinates, size):
    rows_add = []
...
def grid_red_green(green, red, size):
    grid = []
    for y in range(size):
        row = []
        for x in range(size):
...
def main():
    ...
    print(red_coordinates, size, '\n', minimum)
    grid_red = add_red_tiles(red_coordinates, size)
    # grid = add_green_tiles(coordinates, size)
    # print(grid_red)
    green_coords = get_green_coordinates(red_coordinates)
    # print(len(green_coords))
    grid_green = add_green_tiles(green_coords, size)
    green_coords = fill_green_coords(grid_green, green_coords, size)
    grid_green = add_green_tiles(green_coords, size)
    # print(len(green_coords))
    # display_grid(grid_green, size)
    # display_grid(grid_green, size)
    red_green_coord = green_coords + red_coordinates
    red_green_coord = list(set(red_green_coord))
    grid_complete = grid_red_green(green_coords, red_coordinates, size)
    # display_grid(grid_complete, size)
